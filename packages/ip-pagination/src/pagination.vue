...
<template>
  <div ref="changePaginationDom">
    <el-pagination ref="pagination" v-bind="_attrs" :total="total" :current-page="currentPage" :page-sizes="pageSizes"
      :page-size="pageSize" :layout="layout" @size-change="sizeChange" @current-change="currentChange"
...
export default {
  ...
  mounted() {
    const el = this.$refs.changePaginationDom;
    const elPaginationRightWrapperEl = el.querySelector('.el-pagination__rightwrapper') || null;
    if (!elPaginationRightWrapperEl) return;
    const prevButtonEl = el.querySelector('button.btn-prev') || null;
    if (!prevButtonEl) return;
    prevButtonEl.innerHTML = '上一页';
    const nextButtonEl = el.querySelector('button.btn-next') || null;
    ...
    const pageText = el.querySelector('.el-pagination__jump') || null;
    if (!pageText) return;
