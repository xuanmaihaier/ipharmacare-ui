...
export default {
  ...
  props: {
    ...
    layout: {
      default: 'total, ->, sizes, frist,prev, pager, next,last, jumper'
    },
      ...
      render(h) {
        return (
          <button
            ...
            class="btn-frist"
            disabled={ this.$parent.disabled || this.$parent.internalCurrentPage <= 1 }
            on-click={ this.$parent.frist }>
            {
              <span>{ this.$parent.fristText }</span>
            }
          </button>
        );
      }
  ...
  methods: {
    ...
    frist() {
      ...
      const newVal = 1;
      this.internalCurrentPage = this.getValidCurrentPage(newVal);
      this.$emit('frist-click', this.internalCurrentPage);
      this.emitChange();
    },
