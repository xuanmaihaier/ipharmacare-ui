<template>
    ...
    >
        <div class="flex-1 flex flex-column drugContent">
            <div class="flex-1 flex flex-column" v-show="typeModel=='tree'">
                <div class="search-bar tree-dialog-search">
                    ...
                    >
                        ...
                        <el-form-item>
                            <el-button icon="el-icon-search" size="small" plain @click="search"
                                >查询</el-button
                            >
                        </el-form-item>
                    </el-form>
                    <el-button icon="el-icon-edit" size="small" plain @click="goToBatchBox">批量输入</el-button>
                </div>
                <div class="flex tree-box-total">
                    <div class="tree-box flex flex-1 flex-column mr5">
                        <div class="flex checked-handler" v-if="isExpand">
                            <span :class="{ 'disabled': !treeData.length }" size="default" @click="allChecked">全选</span>
                        </div>
            ...
            <div class="flex-1 flex flex-column" v-show="typeModel=='batch'" style="background: #f9f9f9;">
                <div class="zoneList">
                    <label>选择机构：</label>
                    <el-select 
                ...
                <div style="padding: 0 10px;">
                 <el-input
                    ...
                    class="importBox"
                    rows="13"
                    :placeholder='"请输入" + _title + "名称， 多个支持换行或|分隔，最多输入1000个名称"'
                    type="textarea"
                    resize="none"
                />
                </div>
                <div class="text-line">当医院存在多机构且多机构下有同名{{ title }}时，可选择指定的机构</div>
            </div>
        </div>
        <!-- footer -->
        <template #footer class="dialog-footer">
            ...
            <template v-if="typeModel=='batch'">
                ...
                <el-button size="small" type="primary" @click="importTreeData" :disabled="!groupTextarea">导入</el-button>
            </template>
        </template>
    </el-dialog>
</template>
...
export default{
    ...
    methods: {
        ...
        open() {
            ...
            if (this._checkedNodes && this._checkedNodes.length && !this.childrenTreeData.length) {
                this.getBackNewData();
            }
        ...
        getChildrenTree(node) {
            ...
            this.rightTreeList = this.getCommonList(_.cloneDeep(this.childrenTreeData))
            this.rightTreeList = _.unionBy(_.concat(this.rightTreeList, otherList), 'id');
            this.childrenTreeData = this.changeTreeList(_.cloneDeep(this.rightTreeList));
        ...
        allChecked() {
            ...
            let commonList = this.getCommonList(_.cloneDeep(nodesList))
            this.rightTreeList = this.getCommonList(_.cloneDeep(this.childrenTreeData))
            this.rightTreeList = _.unionBy(_.concat(this.rightTreeList, commonList), 'id');
            this.childrenTreeData = this.changeTreeList(_.cloneDeep(this.rightTreeList));
            this.expandIdLists(this.childrenTreeData)
            this.expandList = _.cloneDeep(this.expandList);
        },
        ...
        importTreeData() {
            ...
            let namesArr = this.groupTextarea.replace(/[\r\n]/g, "|").split("|").filter((item) => item.length > 0);
            let params = {
                deptType: this._deptType,
                name: '',
                nameList: namesArr,
...
/deep/ .tree-selector-dialog {
    ...
    textarea:focus, textarea.focus, textarea:not(:disabled):not(.disabled):active, textarea:not(:disabled):not(.disabled).active {
        outline: none;
        border: none;
    }
}
